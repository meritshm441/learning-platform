...
export default function SignupPage() {
  ...
  return (
    <div className="flex flex-col items-center w-full p-4 md:p-8 lg:p-12 max-w-lg gap-8">
      <h1 className="lg:text-3xl text-lg font-medium text-[#404040] text-center">Sign up to get started</h1>
      ...
      <div className="w-full">
        ...
        {submitError && <div className="bg-red-50 text-red-600 p-3 rounded-md mb-4 text-sm">{submitError}</div>}
        ...
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            {!errors.password && watch("password") && (
              <div className="text-xs text-[#666666] mt-1">
                ...
                <ul className="space-y-1 pl-4">
                  <li className={`flex items-center gap-1 ${watch("password")?.length >= 8 ? "text-[#77C053]" : ""}`}>
                    {watch("password")?.length >= 8 && <FaCheck className="h-3 w-3" />}
                    At least 8 characters
                  </li>
                  <li
                    className={`flex items-center gap-1 ${
                      /[A-Z]/.test(watch("password") || "") ? "text-[#77C053]" : ""
                    }`}
                  >
                  ...
                  <li
                    className={`flex items-center gap-1 ${
                      /[@$!%*?&]/.test(watch("password") || "") ? "text-[#77C053]" : ""
                    }`}
                  >
          ...
          <PasswordInput
            ...
            valid={
              watch("confirmPassword") && watch("confirmPassword") === watch("password") && !errors.confirmPassword
            }
          ...
          >
            {isSubmitting ? "Signing up..." : "Signup"} {!isSubmitting && <FaArrowRightToBracket className="h-4 w-4" />}
          </button>
        </form>
